...
- **I18N-7** MUST: every locale has its own feed (`/rss.xml`, `/en/rss.xml`) and its own autodiscovery
  `<link rel="alternate">`. A reader subscribing from one language must not receive the other's posts.
  Feed URLs are built from `getRelativeLocaleUrl(locale, '')` plus the file name — passing the file name
  as the path appends a trailing slash and produces `/rss.xml/`, which does not exist.
